...
export class PartsPriceDetailsComponent implements OnInit {
    ...
    constructor(private userServices: StaffPostAuthService, private activeRouter: ActivatedRoute, public router: Router) {
        ...
        this.isLoading = true;
        this.userServices.getPartsPriceDetails({ pm_id: this.pm_id }).subscribe((rdata: any) => {
            if (rdata.ret_data === 'success') {
                const data = rdata.requestedpart;

                const threshold = Number(this.selectedThreshold || 0);
                const difference = Number(data.pm_new_price || 0) - Number(data.pm_price || 0);
                ...
                if (data.pricing_details?.length) {
                    data.pricing_details.forEach((spare: any) => {
                        ...
                        spare.models.forEach((model: any) => {
                            const updatedKms = this.sortKmSlabs(
                                (model.kms || []).map((km: any) => {
    ...
    //     this.userServices.updateSelectedPrices(payload).subscribe(
    //         (rdata: any) => {
    ...
    /**
     * DYC - Updates selected model prices after confirmation.
     * Shows SweetAlert confirmation before calling API.
     */
    updateSelectedPrices() {
        if (this.buttonClicked) return;

        if (this.selectedModelCodes.length === 0) {
            this.coloredToast('warning', 'Please select at least one model!');
            return;
        }
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'This will update the display price in all related Service Packages.',
            icon: 'warning',
                ...
                if (this.selectedModelCodes.includes(model.modelCode)) {
                    model.kms.forEach((km: any) => {
                        updated_kms.push({
                            ...
                            new_display_price: km.new_display_price,
                            new_markup_price: km.new_markup_price,
                            old_markup: km.markup_price,
                            old_display: km.display_price,
                        });
                    });
                }
            });
            ...
            const payload = {
                ...
                pm_name: this.part.spim_name,
                user_name: atob(atob(localStorage.getItem('us_firstname') || '')),
                updated_kms: updated_kms,
                branch: environment.branch_id,
            };
    ...
    /**
     * DYC - Recalculate all display & markup prices
     * based on selected threshold.
     * Shows success toast after recalculation.
     */
    recalculateWithThreshold() {
        ...
        const difference = Number(this.part.pm_new_price || 0) - Number(this.part.pm_price || 0);
        ...
        this.part.model_codes.forEach((model: any) => {
            ...
            model.kms.forEach((km: any) => {
                ...
                const roundedDisplay = threshold > 0 ? this.roundToThreshold(baseDisplay, threshold) : baseDisplay;
            ...
            model.hasMarkupIssue = model.kms.some((km: any) => km.hasZeroOrNegativeMarkup);
        });
        // Determine toast message based on threshold
        let message = '';
        if (threshold === 0) {
            message = 'Service package prices recalculated (No rounding applied).';
        } else {
            message = `Service package prices recalculated (Rounded to nearest ${threshold} AED).`;
        }
        this.coloredToast('success', message);
    }
    ...
    filteredModels() {
        ...
        return (this.part?.model_codes || []).filter(
            (model: any) => model.spmc_value?.toLowerCase().includes(term) || model.spmc_vin_no?.toLowerCase().includes(term)
        );
    }
    ...
    getProblematicKilometers(model: any): string {
        ...
        return model.kms
            ...
            .map((km: any) => (km.km_id == 1 ? 'Quick Lube' : km.km_value / 1000 + 'K km'))
            .join(', ');
    }
    ...
    hasAnyMarkupIssue(): boolean {
        ...
        return this.part.model_codes.some((model: any) => model.hasMarkupIssue);
    }
    ...
    /**
     * Generates example preview data for threshold section.
     */
    getThresholdExamples() {
        if (!this.part?.model_codes?.length) return null;

        let selectedModel: any = null;
        let selectedKm: any = null;

        for (const model of this.part.model_codes) {
            for (const km of model.kms) {
                const originalDisplay = Number(km.original_display_price || 0);
                const difference = Number(this.part.pm_new_price || 0) - Number(this.part.pm_price || 0);

                const spareQty = Number(model.sp_spare_qty || 1);
                const rawNewPrice = originalDisplay + difference * spareQty;

                const previewThreshold = this.getPreviewThreshold();
                const roundedPreview = this.roundToThreshold(rawNewPrice, previewThreshold);

                // pick first example where rounding actually changes value
                if (roundedPreview !== rawNewPrice) {
                    selectedModel = model;
                    selectedKm = km;
                    break;
                }
        ...
        // fallback
        if (!selectedModel) {
            selectedModel = this.part.model_codes[0];
            selectedKm = selectedModel.kms[0];
        }

        const originalDisplay = Number(selectedKm.original_display_price || 0);
        const difference = Number(this.part.pm_new_price || 0) - Number(this.part.pm_price || 0);

        const spareQty = Number(selectedModel.sp_spare_qty || 1);
        const rawNewPrice = originalDisplay + difference * spareQty;

        const previewThreshold = this.getPreviewThreshold();
        const roundedPreview = this.roundToThreshold(rawNewPrice, previewThreshold);

        const adjustment = roundedPreview - rawNewPrice;
    ...
    /**
     * Returns threshold used only for Example 2 preview.
     * If user selects 0 (No rounding),
     * we still show rounding preview using default threshold (5).
     */
    getPreviewThreshold(): number {
        const threshold = Number(this.selectedThreshold || 0);
        return threshold === 0 ? 5 : threshold; // default preview = 5
    }

    /**
     * DYC - Sort KM slabs:
     * 1️⃣ Quick Lube (km_id == 1) always first
     * 2️⃣ Remaining sorted by km_value ascending
     */
    sortKmSlabs(kms: any[]): any[] {
        if (!kms || !kms.length) return [];

        return kms.sort((a, b) => {
            // Quick Lube first
            if (a.km_id == 1) return -1;
            if (b.km_id == 1) return 1;

            // Then sort by km_value ascending
            return Number(a.km_value) - Number(b.km_value);
        });
    }
