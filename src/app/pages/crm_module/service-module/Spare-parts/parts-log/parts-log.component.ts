...
export class PartsLogComponent implements OnInit {
    ...
    formatLogNotes(note: string): string {
        ...
        return note
            .replace(/Old Price\):\s?([\d,\.]+)/g, 'Old Price): <span class="text-red-600 font-semibold">$1</span>')

            .replace(/New Price\):\s?([\d,\.]+)/g, 'New Price): <span class="text-green-600 font-semibold">$1</span>')

            .replace(/Updated By User:\s?([^\.]+)/g, 'Updated By User: <span class="text-blue-600 font-semibold">$1</span>')

            .replace(/Affected Models are:/g, '<br><br><span class="font-semibold text-gray-700 dark:text-gray-300">Affected Models:</span><br>')
