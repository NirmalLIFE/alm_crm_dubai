...
export class CustomerRevisitAnalysisComponent implements OnInit {
    ...
    getJobCardsData() {
        ...
        this.userServices.getJobCardsAgeData({ start_date: this.start_date, end_date: this.end_date }).subscribe((rData: any) => {
            if (rData.ret_data == 'success') {
                ...
                this.totalCount.totalJobs = this.jobData1 ? this.jobData1.length : 0;
                this.totalCount.totalCustomers = this.jobData ? this.jobData.length : 0;
                if (this.jobData1) {
    ...
    prevYearJobCardsData() {
        ...
        let start_date_parts = this.start_date.split('-');
        let start_date_year = Number(start_date_parts[0]) - 1;
        let start_date_month = Number(start_date_parts[1]) - 1; // Fix: month - 1
        let start_date_day = Number(start_date_parts[2]);
        let StartDate = new Date(start_date_year, start_date_month, start_date_day);

        let end_date_parts = this.end_date.split('-');
        let end_date_year = Number(end_date_parts[0]) - 1;
        let end_date_month = Number(end_date_parts[1]) - 1; // Fix: month - 1
        let end_date_day = Number(end_date_parts[2]);
